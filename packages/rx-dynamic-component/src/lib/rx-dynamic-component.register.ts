...
export class RxDynamicComponentRegister {
    ...
    private readonly _events$ = new Subject<
        DynamicOutputEmission<any, unknown>
    >();
    ...
    registerComponentRef<TComponent = unknown>(
        ...
        if (mirror) {
            ...
            mirror.inputs.forEach((input) => {
                ...
                if (this.inputValues.has(input.propName)) {
                    componentRef.setInput(
                        input.propName,
                        this.inputValues.get(input.propName)
                    );
                }
            });
            ...
            this.logger.error(
                `Could not determine mirror for ${component} to determine inputs and outputs`
            );
        }
    }
    ...
    setInput<T = unknown>(input: string, value: T): void {
        this.logger.log(`Setting ${input} to ${value}`);
        ...
        if (this.inputs.has(input)) {
            ...
            this.logger.warn(
                `${this._component} does not have an input configured with @Input called ${input}.`
            );
        }
    }
