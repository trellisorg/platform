...
export function setupNxBetterer(): NxBettererContext {
    const plainWorkspaceJson =
        readJsonFile<{ projects: Record<string, string | any> }>(
            'workspace.json'
        );

    const workspaceJson = readCachedProjectGraph();

    const workspaceProjects: ProjectGraphProjectNode[] = Object.entries(
        workspaceJson.nodes
    )
        .filter(
            ([, node]) => !node.name.startsWith('npm:') && node.type !== 'e2e'
        )
